...
def predict_wait_time(df: pd.DataFrame, crowd: int, slots: int, hour: int) -> float:
    hist_wait = float(df["queue_minutes"].median())
    hist_vehicles = max(float(df["vehicles"].median()), 1.0)
    hist_slots = max(float(df["slots"].median()), 1.0)
    crowd_pressure = crowd / max(slots, 1)
    baseline_pressure = hist_vehicles / hist_slots
    time_boost = 1.15 if 17 <= hour <= 21 else 1.0
    wait = hist_wait * (0.55 + (crowd_pressure / max(baseline_pressure, 0.5)) * 0.6) * time_boost
...
def predict_charge_time(
    ...
    # Keep real-world adjustments mild so the result stays close to the
    # simple EV bike guidance the app presents to users.
    ...
    # Full-charge guidance for EV bikes:
    # 0-20% -> about 2 hours
    # 30-50% -> about 1 to 1.5 hours
    # 80-90% -> about 20 to 40 minutes
    if battery_level <= 20:
        band_position = battery_level / 20.0
        base_minutes = 120.0 - band_position * 10.0
    elif battery_level <= 30:
        band_position = (battery_level - 20.0) / 10.0
        base_minutes = 110.0 - band_position * 20.0
    elif battery_level <= 50:
        band_position = (battery_level - 30.0) / 20.0
        base_minutes = 90.0 - band_position * 30.0
    elif battery_level <= 80:
        band_position = (battery_level - 50.0) / 30.0
        base_minutes = 60.0 - band_position * 20.0
    elif battery_level <= 90:
        band_position = (battery_level - 80.0) / 10.0
        base_minutes = 40.0 - band_position * 20.0
    else:
        band_position = (battery_level - 90.0) / 10.0
        base_minutes = 20.0 - band_position * 10.0

    adjusted = base_minutes * adjustment
    return round(min(max(adjusted, 8.0), FULL_CHARGE_CEILING_MIN), 1)


def estimate_charge_gain_km(
    target_level: float,
    weather: str,
    temperature: float,
    battery_health: float,
    ...
    result = predict_range(
        ...
        vehicle_base_range_km=vehicle_base_range_km,
        ride_mode=ride_mode,
        terrain=terrain,
        traffic=traffic,
    ...
    return result["predicted_range_km"]
