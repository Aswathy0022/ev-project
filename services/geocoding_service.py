...
def _parse_coordinate_text(place_text: str) -> GeocodeResult | None:
    parts = [part.strip() for part in place_text.split(",")]
    if len(parts) != 2:
        return None
    try:
        lat = float(parts[0])
    ...
    return GeocodeResult(
        ...
        status="success",
        message="Used the coordinates you entered directly.",
    )
...
def _offline_place_lookup(place_text: str) -> GeocodeResult | None:
    ...
    if key in OFFLINE_PLACE_MAP:
        ...
        return GeocodeResult(
            ...
            label=label,
            status="success",
            message=f"Using offline place match: {label}",
        )
    return None
...
def geocode_place(place_text: str) -> GeocodeResult:
    ...
    if not place_text:
        return GeocodeResult(None, None, None, "empty", "Enter a place name to continue.")

    parsed = _parse_coordinate_text(place_text)
    if parsed:
        return parsed

    offline_match = _offline_place_lookup(place_text)
    if offline_match:
    ...
    try:
        ...
        return GeocodeResult(
            ...
            None,
            "missing_dependency",
            "Place search needs the geopy package. Try a common city name, use advanced coordinates, or install geopy.",
        )
    ...
    try:
        ...
        return GeocodeResult(
            ...
            None,
            "network_error",
            "Place search is temporarily unavailable. Check internet access or use advanced coordinates.",
        )
    ...
    if not location:
        return GeocodeResult(
            ...
            None,
            "not_found",
            "That place could not be found. Try a more specific name or use advanced coordinates.",
        )
    ...
    return GeocodeResult(
        ...
        status="success",
        message=f"Using: {location.address}",
    )
